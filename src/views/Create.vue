    ...
    <div class="p-8 flex items-start bg-light-grey rounded-md shadow-lg">
      ...
      <form @submit.prevent="createWorkout" class="flex flex-col gap-y-5 w-full">
        ...
        <div class="flex flex-col">
          ...
          <input v-model="workoutName" type="text" required id="workout-name" class="p-2 text-gray-500 focus:outline-none"/>
        </div>

        <!-- Workout Type -->
        <div class="flex flex-col">
          <label for="workout-type" class="mb-1 text-sm text-at-light-green">Workout Type</label>
          <select @change="workoutChange" v-model="workoutType" required id="workout-type" class="p-2 text-gray-500 focus:outline-none">
        ...
        <div v-if="workoutType === 'strength'" class="flex flex-col gap-y-4">
          <div v-for="(item, index) in exercises" :key="index" class="flex flex-col gap-x-6 gap-y-2 relative md:flex-row">
            <div class="flex flex-col md:w-1/3">
              ...
              <input v-model="item.exercise" type="text" required id="exercise-name" class="p-2 w-full text-gray-500 focus:outline-none"/>
            </div>
            <div class="flex flex-col flex-1">
              <label for="sets" class="mb-1 text-sm text-at-light-green">Sets</label>
              <input v-model="item.sets" type="text" required id="sets" class="p-2 w-full text-gray-500 focus:outline-none"/>
            </div>
            <div class="flex flex-col flex-1">
              <label for="reps" class="mb-1 text-sm text-at-light-green">Reps</label>
              <input v-model="item.reps" type="text" required id="reps" class="p-2 w-full text-gray-500 focus:outline-none"/>
            </div>
            <div class="flex flex-col flex-1">
              <label for="weight" class="mb-1 text-sm text-at-light-green">Weight (KG's)</label>
              <input v-model="item.weight" type="text" required id="weight" class="p-2 w-full text-gray-500 focus:outline-none"/>
            </div>

            <img @click="deleteExercise(item.id)" src="../assets/images/trash-light-green.png" class="h-4 w-auto absolute -left-5 cursor-pointer">
          </div>

          <button @click="addExercise" type="button" class="mt-6 py-2 px-6 rounded-sm self-start text-sm text-white bg-at-light-green 
        ...
        <div v-if="workoutType === 'cardio'" class="flex flex-col gap-y-4">
          <div v-for="(item, index) in exercises" :key="index" class="flex flex-col gap-x-6 gap-y-2 relative md:flex-row">
            <div class="flex flex-col md:w-1/3">
              ...
              <select v-model="item.cardioType" required id="cardio-type" class="p-2 w-full text-gray-500 focus:outline-none">
            ...
            <div class="flex flex-col flex-1">
              ...
              <input v-model="item.distance" type="text" required id="distance" class="p-2 w-full text-gray-500 focus:outline-none"/>
            </div>
            <div class="flex flex-col flex-1">
              <label for="duration" class="mb-1 text-sm text-at-light-green">Duration</label>
              <input v-model="item.duration" type="text" required id="duration" class="p-2 w-full text-gray-500 focus:outline-none"/>
            </div>
            <div class="flex flex-col flex-1">
              <label for="pace" class="mb-1 text-sm text-at-light-green">Pace</label>
              <input v-model="item.pace" type="text" required id="pace" class="p-2 w-full text-gray-500 focus:outline-none"/>
            </div>
        ...
        <button type="submit" class="mt-6 py-2 px-6 rounded-sm self-start text-sm text-white bg-at-light-green 
            duration-200 border-solid border-2 border-at-light-green hover:bg-white 
          hover:text-at-light-green">Record Workout</button>
      </form>
    </div>
  </div>
</template>
